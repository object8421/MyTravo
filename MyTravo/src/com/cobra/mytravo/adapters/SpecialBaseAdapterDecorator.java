...
import com.cobra.mytravo.adapters.SpecialDynamicListView.Swappable;
 ...
 */
public abstract class SpecialBaseAdapterDecorator extends BaseAdapter implements SectionIndexer, SpecialDynamicListView.Swappable {
	...
	public SpecialBaseAdapterDecorator(BaseAdapter baseAdapter) {
		mDecoratedBaseAdapter = baseAdapter;
	}

	public void setAbsListView(AdapterView listView) {
		mListView = listView;

		if (mDecoratedBaseAdapter instanceof SpecialBaseAdapterDecorator) {
			((SpecialBaseAdapterDecorator) mDecoratedBaseAdapter).setAbsListView(listView);
		}

		if (mListView instanceof SpecialDynamicListView) {
			SpecialDynamicListView dynListView = (SpecialDynamicListView) mListView;
	...
	public void notifyDataSetChanged() {
		if (!(mDecoratedBaseAdapter instanceof SpecialArrayAdapter<?>)) {
			// fix #35 dirty trick !
	...
	public void notifyDataSetChanged(Boolean force) {
		if ((force) || (!(mDecoratedBaseAdapter instanceof SpecialArrayAdapter<?>))) {
			// leads to an infinite loop when trying because ArrayAdapter triggers notifyDataSetChanged itself
			mDecoratedBaseAdapter.notifyDataSetChanged();
		}
	}
