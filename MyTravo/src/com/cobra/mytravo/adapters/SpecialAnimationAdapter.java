...
/**
 * A {@link SpecialBaseAdapterDecorator} class which applies multiple {@link Animator}s at once to
 ...
 */
public abstract class SpecialAnimationAdapter extends SpecialBaseAdapterDecorator {
	...
	//HRY Add
	//To record the swipping direction
	//0 --> Notiong
	//>0 --> Right
	//<0 --> Left
	protected int mDirection = 0;
	//Lenovo.Hery1 Add for record last valid position
	protected int lastChildViewIndex = -1;

	public SpecialAnimationAdapter(BaseAdapter baseAdapter) {
		super(baseAdapter);
		...
		if (baseAdapter instanceof SpecialAnimationAdapter) {
			((SpecialAnimationAdapter) baseAdapter).setHasParentAnimationAdapter(true);
		}
	}
	...
	public void reset() {
		...
		if (getDecoratedBaseAdapter() instanceof SpecialAnimationAdapter) {
			((SpecialAnimationAdapter) getDecoratedBaseAdapter()).reset();
		}
	}
	...
	public final View getView(int position, View convertView, ViewGroup parent) {
		...
		//Lenovo.Hery1 Add for Direction caculating BEGIN
				if(lastChildViewIndex == -1){
					mDirection = 1;
					lastChildViewIndex = 0;
				}
				else{
					if(position > 0){
						mDirection = position - lastChildViewIndex;
						lastChildViewIndex = position;
					}		
				}
		//Lenovo.Hery1 Add for Direction caculating END
		if (!mHasParentAnimationAdapter) {
	...
	//HRY CHANGE 
	//In order to start left animation
	//	if (position > mLastAnimatedPosition && mShouldAnimate && !isMeasuringGridViewItem) {
			//mDirection = position-mLastAnimatedPosition;
	...
	private void animateView(int position, ViewGroup parent, View view) {
		...
		if (mDecoratedBaseAdapter instanceof SpecialAnimationAdapter) {
			childAnimators = ((SpecialAnimationAdapter) mDecoratedBaseAdapter).getAnimators(parent, view);
		} else {
			childAnimators = new Animator[0];
		}
		Animator[] animators = getAnimators(parent, view);
		//HRY CHANGE
		//CANCEL THE ALPHA ANIMATION HERE
		Animator alphaAnimator = ObjectAnimator.ofFloat(view, "alpha", 1, 1);
